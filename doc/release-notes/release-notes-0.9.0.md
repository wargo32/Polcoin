...
over /Applications/polcoin-qt (on Mac) or polcoind/polcoin-qt (on Linux).
...
Using the standard "./autogen.sh; ./configure; make" to build polcoin-qt and
polcoind makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 0.9 release is moving away from the polcoind executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running bitcoin daemon to do THIS") was split into a separate
executable, 'polcoin-cli'. The RPC client code will eventually be removed from
polcoind, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let polcoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with polcoind
...
- OS X: Fix polcoin-qt startup crash when clicking dock icon
...
- Add separate polcoin-cli client
