...
We recommend that everybody running prior versions of polcoind/polcoin-qt
...
in your terminal, then install the polcoin-qt package:
  sudo apt-get install polcoin-qt
...
* (Windows only): enable ASLR and DEP for polcoin-qt.exe
* (Windows only): add meta-data to polcoin-qt.exe (e.g. description)
...
* Bash programmable completion for polcoind(1)
