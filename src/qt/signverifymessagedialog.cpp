    ...
    platformStyle(platformStyle)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::PolcoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::PolcoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CPolcoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CPolcoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CPolcoinAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
